...
### **4.3. Реализация веб‑части на CMS Grav**

Для реализации веб‑интерфейса страницы входа, помощи пользователю, а также публикации файлов мастера автонастройки АРМ в качестве CMS используется **Grav** (PHP‑платформа с файловым хранилищем контента).

Основные решения:

- веб‑часть размещается на веб‑сервере Nginx + PHP‑FPM внутри инфраструктуры ФК;
- в Grav реализуются страницы:
    - «вход/помощь» с блоками «Вход», «Авария», «Помощь», «Новости»;
    - страницы с описанием процесса настройки рабочего места;
- через админ‑панель Grav осуществляется управление:
    - файлами дистрибутивов CryptoPro (CSP, плагин),
    - корневыми и промежуточными сертификатами,
    - дистрибутивами Настройщика АРМ (Windows/Linux),
    - текстовой инструкцией по ручной настройке,
    - контентом блоков аварийных оповещений и новостной ленты.

Админ‑панель Grav:

- доступна только из **локальной сети ФК** (ограничение на уровне сети/firewall);
- использует **локальную авторизацию** CMS (без интеграции с внешними IDM);
- администрируется уполномоченным подразделением (ЦОКР / эксплуатация ФК).

### **4.4. Метрики и мониторинг веб‑части**

Метрики по работе веб‑части фиксируются на уровне логов веб‑сервера (Nginx):

- количество скачиваний файлов Настройщика АРМ (по путям скачивания);
- показатели доступности страницы (HTTP‑коды, время ответа);
- ошибки и аномалии (4xx/5xx);
- попытки перехода в ЛК ЭБ по кнопке «Приступить к работе» (метрика уточняется).

Сбор и визуализация метрик выполняется средствами системы мониторинга ФК:

- парсинг логов Nginx и загрузка в **Elasticsearch**;
- построение дашбордов в **Kibana** (доступность веб‑части, количество скачиваний, динамика обращений и др.).
...
- [adr-005](adr/adr-005-grav-and-monitoring.md)
  **Выбор CMS Grav и модель мониторинга веб-части**
